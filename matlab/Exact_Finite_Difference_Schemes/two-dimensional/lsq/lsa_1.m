...
while converge > lsp.pr 
    ...
    fn = @(th) norm(x_t - af(th)) ; % the error function to minimize ( = f )
%     fr = @(th) real(x_t - af(th)) ; % the error function to minimize ( = f )      
%     fi = @(th) imag(x_t - af(th)) ; % the error function to minimize ( = f )      
    
%     lr = @(x0) lsqnonlin(fr, x0); % lsq algo we wish to apply for all theta    
%     li = @(x0) lsqnonlin(fi, x0); % lsq algo we wish to apply for all theta    
...
%     [x_r, resnorm_r, residual_r, exitflag_r ] = cellfun(lr, c_ra, 'UniformOutput', false); 
%     [x_i, resnorm_i, residual_i, exitflag_i ] = cellfun(li, c_ra, 'UniformOutput', false); 
    [x_n, resnorm_n, residual_n, exitflag_n ] = cellfun(ln, c_ra, 'UniformOutput', false);

    x = x_n;    
%     x = x_i;
%     x = x_r;    
%     res = [res ; cell2mat([c_ra x, resnorm_r, exitflag_r])];
%     res = [res ; cell2mat([c_ra x, resnorm_i, exitflag_i])];
