function [ sols, params, As, Bs ] = simulation_intg_theta_2D( theta, sim_param )
%SIMULATION_K_H
...
for j = 1:dtheta
    ...
    sim_param.theta = theta(j);
    sim_param.bessel = @(x) bessel_integral(x,...
        0, pi/2) ;    

%     sim_param.bessel = @(x) bessel_integral(x,...
%         theta(1,j) - fourk,  + fourk) ;

%     sim_param.bessel = @(x) bessel_exact_theta(x, theta(1,j));
