classdef ProblemSolver
    ...
    methods (Access = public)
        ...
        function obj = ProblemSolver(param, scheme, solver, sommerfeld)            
            ...
            % define the parallelisation mode (when building the matrix)
            obj = obj.check_parall_type(param);
        end
...
%                 bs = BasicScheme(obj.param, obj.scheme, bss);                                
                bss = BasicSommScheme( wrapped, obj.sommerfeld );
...
%                 bs = BasicScheme(obj.param, obj.scheme);
            ...
            mb = MatrixBuilder(bs, obj.param.parall);
            
            tic;
            display(sprintf('build the matrix...'));
            [ A, b ] = mb.build();
            building_time = toc;
            display(sprintf('building time: %0.5f', building_time)); 
            tic;
            display(sprintf('solve Ax=b...'));            
            x = obj.solver(A, b);
            solving_time = toc;
            display(sprintf('solving time: %0.5f', solving_time)); 
    ...
    methods(Access = private)        
        ...
        function obj = check_parall_type(obj, param)
            if ~ isfield(param, 'parall')                
                obj.param.parall = 'sequential';
                return;
            end 
            
            parall = param.parall;            
            p = inputParser;            
            % check the building type of the matrix ('sequential',
            % 'parall_1' 'parall_2')
            function res = valide_parall(x)
                res = any(strcmp(x, ...
                    {'sequential', 'parall_1', 'parall_2'}));
            end 
            addRequired(p, 'parall', @(x)valide_parall(x));  
